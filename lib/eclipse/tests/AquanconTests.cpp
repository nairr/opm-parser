...
inline std::vector<Aquancon::AquanconOutput> test_init(){
    Parser parser;
    std::string filename = "./SPE1CASE1_aquancon.data";
    ...
    for (auto it = aquifers.begin(); it != aquifers.end(); ++it){
        std::cout << "Aquifer ID = " << it->aquiferID << ":" << std::endl;
        for (auto itr = it->global_index.begin(); itr != it->global_index.end(); ++itr)
